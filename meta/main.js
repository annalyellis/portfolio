...
function updateScatterplotVisibility(filteredCommits) {
  // Update circle visibility based on filtered commits
  d3.selectAll('circle')
    .style('display', d => {
      return d && filteredCommits.some(commit => commit.id === d.id) ? null : 'none';
...
function onTimeSliderChange() {
  ...
  if (timeScale) {
    ...
    updateScatterplotVisibility(filteredCommits);
    updateFileDisplay(filteredCommits);
  }
}
...
function setupScrollytelling(commits) {
  ...
  function onStepEnter(response) {
    ...
    console.log(stepData.datetime);
    
    // Filter commits up to and including this step's commit
    const stepIndex = commits.findIndex(c => c.id === stepData.id);
    if (stepIndex >= 0) {
      filteredCommits = commits.slice(0, stepIndex + 1);
      updateScatterplotVisibility(filteredCommits);
      updateFileDisplay(filteredCommits);
    }
  }
